...
public class DesktopManager : MonoBehaviour
{
    ...
    private List<Desktop> Monitors = new List<Desktop>();
    ...
    IEnumerator Start()
    {
        ...
        if (screen > nScreen)
        {
            Debug.Log("Error! invalid screen");
            s = 0;
        }
        GameObject monitor = GameObject.Instantiate(monitorBase, position);
        Vector3 localPosition = monitor.transform.localPosition;
        localPosition.z -= screenOffset;
        monitor.transform.localPosition = localPosition;

        monitor.name = "Monitor " + (s + 1).ToString();
        Desktop desk = monitor.AddComponent<Desktop>();
        desk.SetManager(this);
        desk.Screen = s;
        desk.ScreenIndex = s;
    ...
    void Update()
    {
        ...
        foreach (Desktop monitor in Monitors)
        {
            monitor.HideLine();
    ...
    public void Connect(Desktop winDesk)
    {
    ...
    public void Disconnect(Desktop winDesk)
    {
    ...
    private void ReInit()
    {
        ...
        foreach (Desktop winDesk in Monitors)
        {
